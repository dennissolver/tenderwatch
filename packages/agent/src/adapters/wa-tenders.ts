...
export class WATendersAdapter extends BaseSiteAdapter {
  ...
  async login(username: string, password: string): Promise<LoginResult> {
    try {
      ...
      await this.page.waitForSelector('input[name="username"], input[name="email"], #username', { timeout: 10000 });

      const usernameField = await this.page.$('input[name="username"], input[name="email"], #username');
  ...
  async register(params: RegistrationParams): Promise<RegistrationResult> {
    try {
      ...
      const confirmField = await this.page.$('input[name="confirmPassword"], input[name="retypePassword"], #confirmPassword');
      if (confirmField) await confirmField.fill(params.password);

      const companyField = await this.page.$('input[name="organisationName"], input[name="orgName"], #organisationName');
